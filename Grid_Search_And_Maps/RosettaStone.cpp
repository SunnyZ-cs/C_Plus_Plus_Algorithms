...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if (!(kGramLength > 0)) {
        error("Gram length k is not positive. Please enter a positive number.");
    }
    int len = str.length();
    if (len < kGramLength) {
        return Count;
    }
    for (int i = 0; i + kGramLength - 1 < len; i++) {
        Count[str.substr(i, kGramLength)] += 1;
    }
    return Count;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string kGram: input) {
        sum += pow(input[kGram], 2);
    }
    if (sum == 0) {
        error("Input map does not contain any nonzero values.");
    }
    double denom = sqrt(sum);
    Map<string, double> output;
    for (string kGram: input) {
        output[kGram] = input[kGram] / denom;
    }
    return output;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        error("Number of k-grams to keep is negative.");
    }
    ...
    for (string kGram: source) {
        double weight = -1 * source[kGram];
        pq.enqueue(kGram, weight);
    }
    for (int i = 0; i < numToKeep; i++) {
        string kGram = pq.dequeue();
        output[kGram] = source[kGram];
    }
    return output;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    string mostSimLang;
    double mostSimValue = 0;
    for (Corpus lang: corpora) {
        double currentSim = cosineSimilarityOf(textProfile, lang.profile);
        if ( currentSim > mostSimValue) {
            mostSimValue = currentSim;
            mostSimLang = lang.name;
        }
    }
    return mostSimLang;
}
...
/* * * * *   Unit tests curated by Sunny Zhao   * * * * */
STUDENT_TEST("kGramsIn works when the text length exactly matches the k-gram length.") {
...
STUDENT_TEST("kGramsIn works when the text length is one more than the k-gram length.") {
    Map<string, double> expected = {
        { "the", 1 },
        { "hem", 1 }
    };
...
STUDENT_TEST("kGramsIn works on a sample when k = 1.") {
    Map<string, double> expected = {
        { "A", 7 },
        { "B", 2 },
        { "D", 1 },
...
STUDENT_TEST("kGramsIn works on a sample when k = 2.") {
    Map<string, double> expected = {
        { "AB", 2 },
        { "AN", 4 },
        { "BA", 2 },
...
STUDENT_TEST("kGramsIn handles non-English strings.") {
     ...
     * the project description for more details about what's
     * going on here.
     */
...
STUDENT_TEST("kGramsIn works when the input is shorter than the k-gram length.") {
    EXPECT_EQUAL(kGramsIn("ABANANABANDANA", 137), {});
}

STUDENT_TEST("kGramsIn reports errors on bad inputs.") {
    EXPECT_ERROR(kGramsIn("Oops, negative", -137));
    EXPECT_ERROR(kGramsIn("Oops, zero",     0));
}

STUDENT_TEST("kGramsIn works on large inputs.") {
...
STUDENT_TEST("normalize does not add or remove keys.") {
...
STUDENT_TEST("normalize works on positive numbers.") {
...
STUDENT_TEST("normalize works on negative numbers.") {
    Map<string, double> scores = {
        { "C", -1 },
        { "O", -1 },
        { "N", -1 },
...
STUDENT_TEST("normalize works on unequal values.") {
    Map<string, double> scores = {
        { "O",  3 },
        { "N", -4 },
    };
...
STUDENT_TEST("normalize works on huge numbers.") {
    /* As mentioned in the project description, the int type
...
STUDENT_TEST("normalize reports errors on inputs that are all zero.") {
    EXPECT_ERROR(normalize({}));
    EXPECT_ERROR(normalize({{"A", 0}, {"C", 0}, {"E", 0}}));
}

STUDENT_TEST("normalize handles large inputs.") {
...
STUDENT_TEST("topKGramsIn finds the most frequent k-gram.") {
...
STUDENT_TEST("topKGramsIn finds the top two k-grams.") {
    Map<string, double> input = {
        { "A", 10 },
        { "B", 30 },
        { "C", 20 }
...
STUDENT_TEST("topKGramsIn works if all k-grams are requested.") {
    Map<string, double> input = {
        { "A", 10 },
        { "B", 30 },
        { "C", 20 }
...
STUDENT_TEST("topKGramsIn works if more k-grams are requested than exist.") {
    Map<string, double> input = {
        { "A", 10 },
        { "B", 30 },
        { "C", 20 }
...
STUDENT_TEST("topKGramsIn works if no k-grams are requested.") {
    Map<string, double> input = {
        { "A", 10 },
        { "B", 30 },
        { "C", 20 }
...
STUDENT_TEST("topKGramsIn reports errors on bad inputs.") {
    Map<string, double> input = {
        { "A", 10 },
        { "B", 30 },
        { "C", 20 }
...
STUDENT_TEST("topKGramsIn works even if frequencies are negative.") {
    Map<string, double> input = {
        { "A",  2 },
        { "B",  1 },
        { "C",  0 },
...
STUDENT_TEST("topKGramsIn works with fractional weights.") {
    Map<string, double> input = {
        { "A", 10.1 },
        { "B", 10.2 },
        { "C", 10.3 }
...
STUDENT_TEST("topKGramsIn handles ties.") {
...
STUDENT_TEST("topKGramsIn handles large inputs.") {
    /* This next piece of code computes a frequency map
     * consisting of all 8! = 40,320 permutations of the
     * string "abcdefgh". This is a large but managable
     * number of keys to have in a Map that's passed off
...
STUDENT_TEST("cosineSimilarityOf works with one shared key.") {
...
STUDENT_TEST("cosineSimilarityOf works with two maps with non-overlapping keys.") {
    Map<string, double> one = {
        { "A",  1 },
    };
    Map<string, double> two = {
...
STUDENT_TEST("cosineSimilarityOf works where keys are present in RHS but not LHS.") {
...
STUDENT_TEST("cosineSimilarityOf works where keys are present in LHS but not RHS.") {
    /* Yes, we know these aren't normalized and that we promised that we would normalize
     * all inputs to cosineSimilarityOf. This is just for testing purposes.
     */
    Map<string, double> one = {
...
STUDENT_TEST("cosineSimilarityOf works with keys missing from both sides.") {
    /* Yes, we know these aren't normalized and that we promised that we would normalize
     * all inputs to cosineSimilarityOf. This is just for testing purposes.
     */
    Map<string, double> one = {
...
STUDENT_TEST("cosineSimilarityOf works with disjoint keys.") {
...
STUDENT_TEST("cosineSimilarityOf works on large inputs.") {
    /* This next piece of code computes a frequency map
     * consisting of all 8! = 40,320 permutations of the
     * string "abcdefgh". This is a large but managable
     * number of keys to have in a Map that's passed off
...
STUDENT_TEST("guessLanguageOf works with perfect similarity.") {
...
STUDENT_TEST("guessLanguageOf works with imperfect similarity.") {
...
STUDENT_TEST("guessLanguageOf works on more complex inputs.") {
...
STUDENT_TEST("guessLanguageOf reports errors if no corpora are provided.") {
