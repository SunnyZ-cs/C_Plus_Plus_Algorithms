...
/* Magic numbers declared here. */
const int FIREWORKSIZE = 50;
const double VELOCITYLOW = -3;
const double VELOCITYHIGH = 3;
const int LIFELOW = 2;
const int LIFEHIGH = 10;

/* A simple helper function that checks if a given particle is onscreen. */
...
/* A simple helper function that checks if a given particle should continue to exist (i.e.
 * not offscreen or with negative lifetime) */
bool validParticle(double x, double y, int lifetime) {
    if (!isOnScreen(x, y) || lifetime < 0) {
        return false;
    }
...
/* A helper member function that helps remove a particle from the system. */
void ParticleSystem::removeParticle(ParticleCell* temp) {
    ParticleCell* prevHolder = temp->prev;
    ParticleCell* nextHolder = temp->next;
    delete temp;
    numOfParticles--;

    /* 4 possibilities for the state of prevHolder and nextHolder: 11, 01, 10, 00.
     * Case 1 (11): the cell is in the middle of the list. */
    if (prevHolder != nullptr && nextHolder != nullptr) {
        prevHolder->next = nextHolder;
        nextHolder->prev = prevHolder;

    /* Case 2 (01): the cell is the first one in the list. */
    } else if (prevHolder == nullptr && nextHolder != nullptr) {
        nextHolder->prev = nullptr;
        head = nextHolder;

    /* Case 3 (10): the cell is the last one in the list. */
    } else if (prevHolder != nullptr && nextHolder == nullptr){
        prevHolder->next = nullptr;
        tail = prevHolder;

    /* Case 4 (00): the cell is both the first and the last cell in the list
     * (i.e. it's the only cell in the list). After deletion, pointers
     * head and tail should both point to the nullptr, since the list is
     * now empty with no cells at all.
     */
    } else {
        head = tail = nullptr;
    }
...
/* A helper member function that takes in the firework particles's last position and
 * creates many new streamer particles to mimick the explosion. */
void ParticleSystem::explosion(double startX, double startY) {
    /* Checking if the starting position is still on screen is not functionally necessary (because
     * add() already has a checkpoint for this) but it makes sure that the for loop below won't
     * introduce redundancies through performing 50 failed add() (because of the invalidity of the
     * positions of all new particles in the case that the firework particle is already offscreen). */
    if (!isOnScreen(startX, startY)) return;

    Particle data;
    data.type = ParticleType::STREAMER;
    data.x = startX;
    data.y = startY;
    data.color = Color::RANDOM();

    for (int i = 0; i < FIREWORKSIZE; i++) {
        data.dx = randomReal(VELOCITYLOW, VELOCITYHIGH);
        data.dy = randomReal(VELOCITYLOW, VELOCITYHIGH);
        data.lifetime = randomInteger(LIFELOW, LIFEHIGH);
        add(data);
    }
}

/* Constructor that initilizes a new particle system with no particles in it.*/
ParticleSystem::ParticleSystem() {
    head = tail = nullptr;
    numOfParticles = 0;
}

/* Destructor that cleans up all the particles in the system.*/
ParticleSystem::~ParticleSystem() {
    while (head != nullptr) {
...
/* Returns the number of particles in the system. Runtime: O(1) */
int ParticleSystem::numParticles() const {
    return numOfParticles;
}

/* Takes as input a Particle object, then creates a new ParticleCell and wires it
 * onto the back of the linked list. Runtime: O(1)*/
void ParticleSystem::add(const Particle& data) {
    /* Don't add particles that are offscreen or with negative lifetimes. */
    if (!validParticle(data.x, data.y, data.lifetime)) return;
    ...
    /* Needs to handle edge case where list is empty.*/
    if (head == nullptr) {
        head = tail = newCell;
    } else {
        tail->next = newCell;
...
void ParticleSystem::drawParticles() const {
    ParticleCell* temp = head;
    while (temp != nullptr) {
        drawParticle(temp->particle.x, temp->particle.y, temp->particle.color);
        temp = temp->next;
    }
}
...
void ParticleSystem::moveParticles() {
    ParticleCell* temp = head;
    while (temp != nullptr) {
        /* Moves all the particles in the system by the amount specified in their dx and dy fields.*/
        temp->particle.x += temp->particle.dx;
        temp->particle.y += temp->particle.dy;
        temp->particle.lifetime--;

        /* Ballistic and firework types work differently from streamer by changing dy after every move.*/
        if (temp->particle.type != ParticleType::STREAMER) {
            temp->particle.dy++;
        }

        /* Temporary helper to remember what comes next to avoid the inability to access it after the current
         * cell is deleted if it happens to be invalid. */
        ParticleCell* nextHelper = temp->next;

        /* Removes a particle from the system immediately if that particle can't exist anymore. */
        if (!validParticle(temp->particle.x, temp->particle.y, temp->particle.lifetime)) {

            /* When a firework particle's lifetime decreases below zero, it's now eligible for explosion. */
            if (temp->particle.type == ParticleType::FIREWORK && temp->particle.lifetime < 0) {
                explosion(temp->particle.x, temp->particle.y);
            }

            /* Ordering here makes sure all necessary reading actions are done before cell is deleted. */
            removeParticle(temp);
        }

        /* The pointer temp now advances one step forward to look at the next item in the list.*/
        temp = nextHelper;
    }
}

/* * * * * Unit tests curated by Sunny Zhao * * * * */
STUDENT_TEST("Milestone 1: Checks that values of newly introduced member variables are correct.") {
    ...
    /* Invasively check to make sure the member variable numOfParticles
     * holds the correct value, in this case it should be one. */
    EXPECT_EQUAL(system.numOfParticles, 1);

    /* Invasively check to make sure the member pointer tail isn't null,
     * since it needs to hold our particle.
     */
    EXPECT_NOT_EQUAL(system.tail, nullptr);
}

STUDENT_TEST("Milestone 5: All new particles are emitted from the firework's last position in an explosion.") {
    ParticleSystem system;
    ...
    /* All of the new particles created should be positioned at
     * the firework's last position.
     */
    for (auto* curr = system.head; curr != nullptr; curr = curr->next) {
        EXPECT_EQUAL(curr->particle.x, 100);
        EXPECT_EQUAL(curr->particle.y, 100);
    }
}
...
STUDENT_TEST("Milestone 1: Constructor creates an empty particle system.") {
    ParticleSystem system;
    EXPECT_EQUAL(system.head, nullptr);
}

STUDENT_TEST("Milestone 1: Empty system has no particles.") {
    ParticleSystem system;
    EXPECT_EQUAL(system.numParticles(), 0);
}

STUDENT_TEST("Milestone 1: Can add a single particle.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 1: Can add two particles.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 1: Can add multiple particles.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 1: Stress Test: Efficiently adds and counts particles.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 2: Single particle gets drawn.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 2: Multiple particles are drawn in order.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 3: Can move a single particle.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 3: Can move multiple particles with different velocities.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 4: Don't add invalid particles to the list.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 4: Particle removed when it leaves the screen.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 4: Particle removed when its lifetime ends.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 4: All particles move even if first needs to be removed.") {
...
STUDENT_TEST("Milestone 4: All particles move even if second needs to be removed.") {
    /* Create a list of five particles at positions (0, 0), (1, 0), ... (4, 0).
     * All particles move downward. All particles have a long lifetime except
...
STUDENT_TEST("Milestone 4: All particles move even if last needs to be removed.") {
    /* Create a list of five particles at positions (0, 0), (1, 0), ... (4, 0).
     * All particles move downward. All particles have a long lifetime except
...
STUDENT_TEST("Milestone 4: All particles move even if second-to-last needs to be removed.") {
    /* Create a list of five particles at positions (0, 0), (1, 0), ... (4, 0).
     * All particles move downward. All particles have a long lifetime except
...
STUDENT_TEST("Milestone 4: All particles move even if many need to be removed.") {
    /* Create a list of five particles at positions (0, 0), (1, 0), ... (4, 0).
...
STUDENT_TEST("Milestone 4: After all particles expire, can add new particles.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 5: Ballistic particles accelerate downward.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 5: Fireworks accelerate downward.") {
    ParticleSystem system;
...
STUDENT_TEST("Milestone 5: Fireworks explode when timer expires.") {
    ParticleSystem system;
