...
void prependTo(Cell*& list, const string& value) {
    ...
    /* Needs to handle edge case where the initial list is empty.*/
    if (list == nullptr) {
        list = newCell;
    } else {
        list->prev = newCell;
...
STUDENT_TEST("lengthOf: Empty list.") {
    EXPECT_EQUAL(lengthOf(nullptr), 0);
}

STUDENT_TEST("lengthOf: List of length 1.") {
...
STUDENT_TEST("lengthOf: List of length 2.") {
...
STUDENT_TEST("lengthOf: List of length 3.") {
...
STUDENT_TEST("freeDoublyLinkedList: Empty list.") {
    freeDoublyLinkedList(nullptr); // Should do nothing
}

STUDENT_TEST("freeDoublyLinkedList: List of length 1.") {
    /* Create a one-element list. */
    Cell* list = new Cell;
    list->next = list->prev = nullptr; // Cell has nothing before or after it.
...
STUDENT_TEST("freeDoublyLinkedList: List of length 2.") {
    /* Create two cells and wire them together. */
    Cell* one = new Cell;
    Cell* two = new Cell;
    one->next = two;
...
STUDENT_TEST("freeDoublyLinkedList: List of length 3.") {
    /* Create three cells and wire them together. */
    Cell* a = new Cell;
    Cell* b = new Cell;
    Cell* c = new Cell;
...
STUDENT_TEST("prependTo: Prepend to empty list.") {
    Cell* list = nullptr;
    prependTo(list, "A");
...
STUDENT_TEST("prependTo: Prepend to length-1 list.") {
    /* Create a one-element list. */
    Cell* list = new Cell;
    list->next = list->prev = nullptr; // Cell has nothing before or after it.
    list->value = "B";
...
STUDENT_TEST("prependTo: Prepend to length-2 list.") {
    /* Create two cells and wire them together. */
    Cell* one = new Cell;
    Cell* two = new Cell;
    one->next = two;
...
STUDENT_TEST("prependTo: Prepend to length-3 list.") {
    /* Create three cells and wire them together. */
    Cell* a = new Cell;
    Cell* b = new Cell;
    Cell* c = new Cell;
