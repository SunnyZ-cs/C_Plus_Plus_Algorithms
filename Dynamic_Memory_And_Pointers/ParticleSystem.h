...
private:
    ...
    /* Pointer to the last cell in the list of particles, or nullptr if there
     * are no particles in the list.
     */
    ParticleCell* tail;

    /* Tracks the total number of particles in the system. */
    int numOfParticles;

    /* Helps remove a particle from the system by reconfiguring the doubly-linked list. */
    void removeParticle(ParticleCell* temp);

    /* Helps mimick an explosion by adding new streamer particles to the system. */
    void explosion(double startX, double startY);
