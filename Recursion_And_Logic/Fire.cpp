...
void coolDown(Grid<int>& fire, int x1, int y1) {
    double p = double(2) / 3;
    if (fire[y1][x1] != 0 && randomChance(p)) {
        fire[y1][x1] -= 1;
    }
}

void copyTemp(Grid<int>& fire, int x, int y, int x1, int y1) {
    fire[y1][x1] = fire[y][x];
    coolDown(fire, x1, y1);
}

void threeCellsAbove(Grid<int>& fire, int x, int y) {
    int n = randomInteger(0, 2);
    if (n == 0) {
        copyTemp(fire, x, y, x - 1, y - 1);
    } else if (n == 1) {
        copyTemp(fire, x, y, x, y - 1);
    } else {
        copyTemp(fire, x, y, x + 1, y - 1);
    }
}

void twoCellsAbove(Grid<int>& fire, int x, int y, int x1) {
    int n = randomInteger(0, 1);
    if (n == 0) {
        copyTemp(fire, x, y, x, y - 1);
    } else {
        copyTemp(fire, x, y, x1, y - 1);
    }
}

void updateFire(Grid<int>& fire) {
    int col = fire.numCols();
    int row = fire.numRows();
    for (int y = 1; y < row; y++) {
        for (int x = 0; x < col; x++) {
            bool upleft = fire.inBounds(y - 1, x - 1);
            bool upright = fire.inBounds(y - 1, x + 1);
            if (upleft && upright) {
                threeCellsAbove(fire, x, y);
            } else if (upleft && !upright) {
                twoCellsAbove(fire, x, y, x - 1);
            } else if (!upleft && upright) {
                twoCellsAbove(fire, x, y, x + 1);
            } else {
                copyTemp(fire, x, y, x, y - 1);
...
STUDENT_TEST("updateFire does not change dimensions of world.") {
    const int numRows = 5;
    const int numCols = 7;
...
STUDENT_TEST("updateFire does not change bottom row.") {
    Grid<int> fire = {
        { 3, 3, 3 },
        { 5, 5, 5 },
        { 7, 7, 7 }
...
STUDENT_TEST("updateFire copies heat values upward.") {
    Grid<int> fire = {
        { 0, 0, 0, 0, 0, 0, 0, 0 },
        { 4, 4, 4, 4, 4, 4, 4, 4 },
        { 9, 9, 9, 9, 9, 9, 9, 9 }
...
STUDENT_TEST("updateFire copies zero values upward.") {
    Grid<int> fire = {
        { 0, 0, 0, 0, 0, 0, 0, 0 },
        { 4, 4, 4, 4, 4, 4, 4, 4 },
        { 0, 0, 0, 0, 0, 0, 0, 0 }
...
STUDENT_TEST("updateFire shifts values horizontally with the correct probabilities.") {
...
STUDENT_TEST("updateFire cools fire with correct probability.") {
    /* This fire has one heat source at the bottom. We pick a seven-column world
     * so that we can ignore effects on the boundary.
     */
    Grid<int> pattern = {
...
STUDENT_TEST("Fire propagates upward near world boundaries.") {
...
STUDENT_TEST("Fire propagates upward in a narrow world.") {
