...
/* Constructor: create a table filled with empty slots and store the hash function
 * for later use. */
...
/* Destructor cleans up memory used.*/
LinearProbingHashTable::~LinearProbingHashTable() {
    delete[] elems;
}

/* Conveniently returns the number of elements in the table with O(1) runtime.*/
...
/* Insert an element into our table. If the table already contains the element, it
 * will not be added (to avoid duplicates). If the table is already full, it cannot
 * be added (because there is no space anymore). In all other cases, we can add the
 * element.*/
bool LinearProbingHashTable::insert(const string& elem) {
    if (contains(elem)) return false;
    if (numElems == numSlots) return false;

    /* Our table is not full (i.e. with at least an empty space/tombstone where an element
     * can be added), which means that for sure we can add this element to our table. The
     * program will always exit (by the return true; line) the while loop after
     * some finite number of iterations without causing an infinite loop.
     */
    int slotIdx = hashHolder(elem);
    while (true) {
        if (elems[slotIdx].type != SlotType::FILLED) {
            elems[slotIdx].type = SlotType::FILLED;
            elems[slotIdx].value = elem;
            numElems++;
            return true;
        } else {
...
/* Checks if an element is contained in the table.*/
bool LinearProbingHashTable::contains(const string& elem) const {
    int slotIdx = hashHolder(elem);
    for (int i = 0; i < numSlots; i++) {
        if (elems[slotIdx].type == SlotType::EMPTY) {
            return false;
        } else if (elems[slotIdx].type == SlotType::FILLED &&
                   elems[slotIdx].value == elem) {
            return true;
        } else {
            slotIdx = (slotIdx + 1) % numSlots;
    ...
    /* Looked at all slots in the table but didn't find the element. Our table doesn't
     * contain the element.
     */
    return false;
}

/* Removes an element from the table only if it does exist in the table at the first
 * place.*/
bool LinearProbingHashTable::remove(const string& elem) {
    if (!contains(elem)) return false;

    /* The table must contain our element, which means that for sure we can remove this element
     * from our table. The program will always exit (by the return true; line) the while loop after
     * some finite number of iterations without causing an infinite loop.
     */
    int slotIdx = hashHolder(elem);
    while (true) {
        if (elems[slotIdx].type == SlotType::FILLED && elems[slotIdx].value == elem) {
            elems[slotIdx].type = SlotType::TOMBSTONE;
            numElems--;
            return true;
        } else {
            slotIdx = (slotIdx + 1) % numSlots;
...
STUDENT_TEST("Table is initially empty.") {
...
STUDENT_TEST("Can insert and look up a single value.") {
...
STUDENT_TEST("Is case-sensitive.") {
...
STUDENT_TEST("Insertions/lookups work with hash collisions.") {
...
STUDENT_TEST("Insertions/lookups succeed when ranges overlap.") {
...
STUDENT_TEST("Wraps around the end of the table.") {
...
STUDENT_TEST("Doesn't allow for duplicates.") {
...
STUDENT_TEST("Handles inserting the empty string.") {
    LinearProbingHashTable table(Hash::zero(10));
...
STUDENT_TEST("Lookups work even if the table is full.") {
...
STUDENT_TEST("Won't insert elements if table is full.") {
...
STUDENT_TEST("Stress Test: Handles pure insertion of elements (should take at most three seconds).") {
...
STUDENT_TEST("Stress Test: Inserts/searches work in expected time O(1) (should take at most three seconds).") {
...
STUDENT_TEST("Can insert and remove a single element.") {
...
STUDENT_TEST("Handles a single tombstone.") {
...
STUDENT_TEST("Handles lookups with long chains of tombstones") {
    /* Hash function mapping each item to its numeric position, which makes the test
     * predictably control where elements go.
     */
    LinearProbingHashTable table(Hash::identity(10));
...
STUDENT_TEST("Recycles space from tombstones.") {
    LinearProbingHashTable table(Hash::zero(10));
...
STUDENT_TEST("Insertions over tombstones don't add duplicates.") {
...
STUDENT_TEST("Handles removing the empty string.") {
    LinearProbingHashTable table(Hash::random(10));
...
STUDENT_TEST("Can remove from a full table.") {
...
STUDENT_TEST("Stress Test: Handles large numbers of removals (should take at most three seconds).") {
    const int kNumTrials = 50; // Do this lots of times to smoke out any errors that might be lurking.
    for (int trial = 0; trial < kNumTrials; trial++) {
        LinearProbingHashTable table(Hash::random(100));
...
STUDENT_TEST("Stress Test: Inserts/searches/deletes work in expected time O(1) (should take at most three seconds).") {
    /* Huge number of slots. */
    const int kNumSlots = 1000000;
...
#include "filelib.h"
STUDENT_TEST("Stress test: Core functions do not cause stack overflows (should take at most 15 seconds)") {
...
#include <fstream>
STUDENT_TEST("Stress Test: Handles large workflows with little free space (should take at most fifteen seconds)") {
    /*SHOW_ERROR("Stress test is disabled by default. To run it, comment out line " + to_string(__LINE__) + " of " + getTail(__FILE__) + ".");*/
