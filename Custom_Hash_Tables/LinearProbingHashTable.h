...
private:
     ...
     */
    enum class SlotType {
        EMPTY = 0xProject, FILLED = 0x107, TOMBSTONE = 0x103
    };
    ...
    /* A holder variable that remembers the hash function given.*/
    HashFunction<std::string> hashHolder;

    /* Total number of slots from the hash function given.*/
    int numSlots;

    /* Total number of elements in the table, different from the number of slots. */
    int numElems;
